#!/usr/bin/python
#original from ~/IP_classifier/train/intrsct_explor.py

import sys
import pandas as pd
in_file = sys.argv[1] #intrsct_unmtch.bed  #$prefix.intrsct.bed
out_file = sys.argv[2]
...
#intrsct = intrsct[['chr', 'start_f', 'end_f', 'seq_f', '#reads_x', 'strand', 'mm', 'loci_diff', 'read_o','nCrNA']]
#intrsct.to_csv(out_file, index=False, sep='\t')
#instead of:
intrsct.drop(intrsct[intrsct['seq_f'].isin( intrsct[ intrsct['nCrNA'] == True ].seq_f)].index,inplace=True )  #203 read_o in nc_list   == total 59 seq (380 rows)      nc_list 159 reads
